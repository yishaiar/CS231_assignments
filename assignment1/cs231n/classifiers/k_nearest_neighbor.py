...
class KNearestNeighbor(object):
  ...
  def compute_distances_two_loops(self, X_test):
    """
    Compute l2 distance between each test point in X_test and each training point
  ...
  def compute_distances_no_loops(self,  X_test):
    ...
    # (x-y)^2 = x^2+y^2 -2xy
    xx = np.sum(self.X_train**2, axis=1, keepdims=True).T
    # print(xx.shape)
    yy = np.sum(X_test**2, axis=1, keepdims=True)
    # print(yy.shape)
    xy = X_test.dot(self.X_train.T)
    # print(xy.shape)
  ...
  def predict_labels(self, dists, k):
    ...
    for i in xrange(num_test):
      ...
      # all the train distances  from the i'th test  image:
      # single minimal distance; k=1
      # min_index = np.argmin(dists[i,:])
      #  y_pred[i] = self.y_train[min_index]

      # multiple k distances:
